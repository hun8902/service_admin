...
if(isset($_POST["action"]))
{
    ...
    {
        $order_column = array('user_name', 'phone1', 'phone2', 'center_name', 'addr');
    ...
    if($_POST["action"] == 'doc_copy')
    {
        ...
        $data = array(
            ':post_code'			=>	$post_uid,
            ':select_name'			=>	$object->clean_input($_POST["select_name"]),
            ...
            ':frist_user'			=>	$object->clean_input($_POST["frist_user"]),
            ':last_uesr'			=>	$object->clean_input($_POST["last_uesr"]),
    ...
    if($_POST["action"] == 'fetch_single')
    {
        ...
        foreach($result as $row)
        {
            ...
            $data['idx'] = $row['idx'];
            $data['select_name'] = $row['select_name'];
            ...
            //카테고리 테이블 날짜 가져오기
            $object->query = "SELECT * FROM category where parent_id = '0' and id_menu = '".$row['step1']."'";
            $last_date = $object->get_result();
            foreach ($last_date as $row3) {
                $start_date_value = $row3['prev_date'];
                $last_date_value = $row3['update_date'];
            }

            //카테고리 테이블에 업데이트 날짜가 없으면 데이터 없는걸로 간주함
            if($_POST["action"] != 'fetch_single' ){
                //접수 추가시 서비스 유형이 비어있으면 가장 최신의 카테고리 정보를 가져옴
                ...
                foreach ($select_query_load as $row1) {
                    $data['step0_text'] .= '<option id="' . $start_date_value . '" class="' . $last_date_value . '" value="' . $row1["id_menu"] . '"  >' . $row1["label_menu"] . '</option>';
                }
            }else{
                //팝업창 클릭했을떄 날짜를 이용하여 리스트 정보를 가져옴
                $object->query = "SELECT * FROM category where parent_id = '0' and prev_date = '".$start_date_value."' and update_date = '".$last_date_value."'";
                $select_query_load = $object->get_result();
                foreach ($select_query_load as $row1) {
                    ...
                    $data['step0_text'] .= '<option id="' . $row1["id_menu"] . '" value="' . $row1["id_menu"] . '" ' . $select_choice . ' >' . $row1["label_menu"] . '</option>';
                }
            }
        ...
        foreach($result as $row => $value)
        {
            ...
            <div class="form-row" data-repeater-item>
                <div class="col-md-6 mb-3">
                    <select name="group-a['.$row.'][material1]"  class="form-control">
                        <option value="'.$value['group_data1'].'">'.$value['group_data1'].'</option>
                         '.$material_options.'
                    </select>
                </div>
                <div class="col-md-3 mb-3">
                    <input type="number"  min="0" max="999"  name="group-a['.$row.'][material2]" value="'.$value['group_data2'].'" class="form-control" placeholder="수량을 입력해주세요"/>
                </div>
                <div class="col-md-3 mb-3">
    ...
    if($_POST["action"] == 'delete')
    {
        ...
        if((!empty($data['stats_fd1']) || !empty($data['stats_fd2']) || !empty($data['stats_fd3']) || !empty($data['stats_fd4']) || !empty($data['stats_fd5']) || !empty($data['stats_fd2']) || !empty($data['date_1']) || !empty($data['date_2']) || !empty($data['date_3']) || !empty($data['date_4']) || !empty($data['date_5']) || !empty($data['date_6'])) && $level_check == "1"){
            //해당 필드에 값이 있는 경우 옮음만 삭제 가능
            $object->query = "
            DELETE FROM post
            WHERE idx = '".$_POST["id"]."'
            ";
